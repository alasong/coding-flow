...
【通用提示词模板】
当需要提取领域实体时，请仅关注功能需求中的核心名词，严禁联想。
当设计系统组件时，请基于领域实体，每个组件必须有明确的职责和对应的实体。
当融合技术栈时，请严格遵守用户选定的技术栈，不得擅自更改。
当设计架构机制时，请使用标准的设计模式名称（如 Circuit Breaker, Saga Pattern），并结合具体技术栈说明落地实现。
